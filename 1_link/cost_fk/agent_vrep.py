...
class AgentVREP(Agent):
    ...
    def _connect_to_vrep(self):
        ...
        if self.clientID != -1:
            ...
            _, self.targetHandle = vrep.simxGetObjectHandle(self.clientID, 'target', vrep.simx_opmode_blocking)
            vrep.simxSynchronous(self.clientID, True)  # Enable the synchronous mode (Blocking function call)
        else:
            print('no connection')

        # set time-step
        #vrep.simxSetFloatingParameter(self.clientID, vrep.sim_floatparam_simulation_time_step, self._hyperparams['dt'],
    ...
    def restart_simulation(self):
        ...
        vrep.simxSetJointPosition(self.clientID, self.jointHandles[0], self.x0[0][0], vrep.simx_opmode_blocking)
        vrep.simxSetObjectFloatParameter(self.clientID, self.jointHandles[0], 2012, self.x0[0][1], vrep.simx_opmode_blocking)
        vrep.simxSetObjectPosition(self.clientID, self.tipHandle, -1, self.x0[0][2:5], vrep.simx_opmode_blocking)
        vrep.simxSetObjectPosition(self.clientID, self.targetHandle, -1, self._hyperparams['target_end_effector'], vrep.simx_opmode_blocking)
    ...
    def retrieve_state(self):
        ...
        _, _1, jac_list, _2, _3 = vrep.simxCallScriptFunction(self.clientID, 'Jacobian_Script',
